...
export class UserSelect
    constructor:(opts) ->
        ...
        @parentEl.classList.add("user-select-container")
        
        @inputEl = document.createElement("INPUT")
        @inputEl.setAttribute("type", "text")
        @inputEl.setAttribute("placeholder", "Benutzer suchen...")
    ...
    setAllOptions: (opts) =>
        ...
        invalidShapeError = new Error('Options must be a valid array of shape: [["...", "..."], ["...", "..."], ... ] !')

        if !Array.isArray(opts) then throw invalidShapeError
        for opt in opts
            if !Array.isArray(opt) then throw invalidShapeError
            if typeof opt[0] != "string" then throw invalidShapeError
            if typeof opt[1] != "string" then throw invalidShapeError
            @allOptions.push([opt[0], opt[1]])
        return
    ...
    updateShownOptions: ->
        ...
        if query.length == 0
            ...
            @isQuerying = false
            @restartQuery = false
            @shownOptions = @allOptions.slice(0, @optionsLimit)
    ...
    executeAllSymbolsQuery: =>
        ...
        loop
            ...
            for opt,i in @allOptions
                ...
                emailScore = fuzzyScore(@query, opt[0].toLowerCase())
                nameScore = fuzzyScore(@query, opt[1].toLowerCase())
                if emailScore > nameScore then score = emailScore
